Ficha de classificação · Bloco $7$ · Achar o caminho

Ficha de produção (uso interno), no modelo do §$3$ do Protocolo_OBMEP_v$5.5$. Cobre as três questões de referência do bloco: `2018/Q5` (código `NA18-Q05`), `2022/F1/Q13` (código `M2-22-F1-Q13`) e `2025/F1/Q3` (código `M2-25-F1-Q03`).

***Classificação comum às questões do bloco***

**Motor cognitivo (Apêndice A, uso interno).** Rastreamento de trajeto: seguir um percurso, uma sequência de viradas ou um trajeto físico com atenção, sem se deixar enganar pela aparência do desenho nem pelos cruzamentos visuais. O bloco cobre três variações: (a) comparar o comprimento de rotas diferentes somando seus trechos, (b) achar a direção final de um percurso contando viradas à esquerda e à direita e cancelando pares, e (c) seguir um trajeto que se cruza com outros no papel, sem confundir um trajeto com outro no ponto de cruzamento.

**Nó principal (Apêndice D, grade oficial).** `Geometria › Percepção Espacial no Plano › Lateralidade e Rastreamento de Trajetos` (subnó proposto). No Apêndice D, "Geometria" está marcado `[+]`, sem filhos transcritos, e este subnó fica lado a lado com os subnós dos demais Blocos deste Caminho dentro de "Percepção Espacial no Plano".

**Descritores secundários.**
- Comparação de somas (variação a): parente direto do descritor "somar pedaços até um total", já validado no Bloco $2$ do Caminho $3$ (percursos e alturas), aqui aplicado à comparação entre duas rotas diferentes, não a uma só.
- Cancelamento de pares opostos (variação b): contar viradas à esquerda e à direita e cancelar pares é parente do descritor "saldo/entra e sai com total constante" já validado na trilha, aqui aplicado a um giro angular em vez de uma quantidade.
...
Os $10$ exercícios de `Lista_Exercicios_Lateralidade_Caminhos.md` seguem a classificação acima, variando o motor específico dentro do bloco:
- Exercícios $1$, $4$, $7$, $10$: variação (a), comparação de rotas por soma de trechos.
- Exercícios $2$, $5$, $8$: variação (b), direção final por contagem de viradas.
- Exercícios $3$, $6$, $9$: variação (c), rastreamento de trajeto cruzado.
