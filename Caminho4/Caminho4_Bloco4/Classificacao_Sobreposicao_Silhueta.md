Ficha de classificação · Bloco $4$ · O que fica por cima

Ficha de produção (uso interno), no modelo do §$3$ do Protocolo_OBMEP_v$5.5$. Cobre as três questões de referência do bloco: `2018/Q7` (código `NA18-Q07`), `2018/Q11` (código `NA18-Q11`) e `2025/F1/Q6` (código `M2-25-F1-Q06`).

***Classificação comum às questões do bloco***

**Motor cognitivo (Apêndice A, uso interno).** Sobreposição e oclusão: quando figuras ficam umas sobre as outras, ou quando camadas transparentes se combinam, o resultado final se descobre raciocinando sobre o que tapa o quê, não olhando a cena como um desenho só. O bloco cobre três variações: (a) descobrir a ordem de uma pilha de figuras contando quantas bordas de cada uma estão cortadas por outra, (b) descobrir se a sobreposição de folhas com buraco resulta numa superfície totalmente tapada, comparando as posições dos buracos de cada folha, e (c) descobrir a silhueta de duas ou mais figuras coladas no mesmo centro, sem girar, seguindo sempre a figura que se estica mais longe em cada direção.

**Nó principal (Apêndice D, grade oficial).** `Geometria › Percepção Espacial no Plano › Sobreposição, Transparência e Silhueta` (subnó proposto). No Apêndice D, "Geometria" está marcado `[+]`, sem filhos transcritos, e este subnó fica lado a lado com os subnós dos Blocos $1$, $2$ e $3$ deste Caminho dentro de "Percepção Espacial no Plano".

**Descritores secundários.**
- Contagem de oclusão como critério de ordenação: contar bordas cortadas para ordenar uma pilha é parente do descritor "ordenação por pistas" já usado em Raciocínio Lógico, aqui aplicado à posição de camadas visuais, não a comparações textuais.
- Combinação lógica ponto a ponto (transparência): decidir se um ponto fica branco ou preto na sobreposição de duas folhas segue a mesma lógica de "só vale se as duas condições valem ao mesmo tempo", parente do raciocínio de interseção de conjuntos.
...
Os $10$ exercícios de `Lista_Exercicios_Sobreposicao_Silhueta.md` seguem a classificação acima, variando o motor específico dentro do bloco:
- Exercícios $1$, $4$, $7$, $10$: variação (a), ordem de pilha por contagem de bordas cortadas.
- Exercícios $2$, $5$, $8$: variação (b), combinação de folhas transparentes com buraco.
- Exercícios $3$, $6$, $9$: variação (c), silhueta de figuras coladas no centro.
