Ficha de classificação · Bloco $6$ · Achar a peça que encaixa

Ficha de produção (uso interno), no modelo do §$3$ do Protocolo_OBMEP_v$5.5$. Cobre as quatro questões de referência do bloco: `2019/Q5` (código `NA19-Q05`), `2022/F1/Q4` (código `M2-22-F1-Q04`), `2022/F1/Q1` (código `M2-22-F1-Q01`) e `2025/F1/Q11` (código `M2-25-F1-Q11`).

***Classificação comum às questões do bloco***

**Motor cognitivo (Apêndice A, uso interno).** Encaixe e complemento de formas: reconhecer qual peça, entre várias candidatas, fecha exatamente um espaço vazio, uma fechadura ou uma posição de quebra-cabeça, conferindo formato, tamanho, ordem e orientação ao mesmo tempo. O bloco cobre quatro variações: (a) achar o pedaço com contorno exatamente complementar a um buraco, (b) comparar o perfil de uma chave com o perfil de várias fechaduras, dente a dente e na ordem certa, (c) reconhecer que uma peça de quebra-cabeça encaixada virada muda o desenho final, e (d) contar quantas figuras inteiras se formam quando peças precisam casar forma e cor ao mesmo tempo.

**Nó principal (Apêndice D, grade oficial).** `Geometria › Percepção Espacial no Plano › Encaixe e Complemento de Formas` (subnó proposto). No Apêndice D, "Geometria" está marcado `[+]`, sem filhos transcritos, e este subnó fica lado a lado com os subnós dos demais Blocos deste Caminho dentro de "Percepção Espacial no Plano".

**Descritores secundários.**
- Comparação atributo a atributo, na ordem: a variação (b) exige comparar cada dente da chave com a profundidade correspondente da fechadura, na mesma posição, não só os números soltos — parente do descritor "comparação posicional" já usado noutros blocos.
- Restrição dupla simultânea (variação d): encaixar peças por forma e cor ao mesmo tempo é parente do descritor "duas condições que precisam valer juntas" já usado em Raciocínio Lógico.
...
Os $10$ exercícios de `Lista_Exercicios_Encaixe_PecaFaltante.md` seguem a classificação acima, variando o motor específico dentro do bloco:
- Exercícios $1$, $5$, $9$: variação (a), pedaço complementar a um buraco.
- Exercícios $2$, $6$, $10$: variação (b), silhueta complementar de chave e fechadura.
- Exercícios $3$, $7$: variação (c), orientação em quebra-cabeça.
- Exercícios $4$, $8$: variação (d), encaixe por forma e cor, contagem por pareamento.
