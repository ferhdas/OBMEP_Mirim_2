Ficha de classificação · Bloco $2$ · Quebrar e remontar figuras

Ficha de produção (uso interno), no modelo do §$3$ do Protocolo_OBMEP_v$5.5$. Cobre as quatro questões de referência do bloco: `2022/F1/Q14` (código `M2-22-F1-Q14`), `2024/F2/Q14` (código `M2-24-F2-Q14`), `2025/F1/Q14` (código `M2-25-F1-Q14`) e `2024/F2/Q15` (código `M2-24-F2-Q15`).

***Classificação comum às questões do bloco***

**Motor cognitivo (Apêndice A, uso interno).** Área como soma de pedaços: a área de uma figura recortada, ladrilhada ou dividida em regiões é sempre a soma das áreas dos pedaços que a compõem, calculada por contagem de quadradinhos de uma malha. O bloco cobre três variações: (a) contar quantos quadradinhos de referência saem de uma folha recortada, multiplicando largura por altura, (b) achar quantas peças de uma cor faltam num ladrilhamento, dividindo a área total pela área de uma peça e subtraindo as cores já conhecidas — incluindo o caso de reconhecer, sem girar nem virar, qual peça solta encaixa numa vaga de formato e orientação fixos — e (c) achar a área de uma região que falta, subtraindo do total a soma das áreas já conhecidas.

**Nó principal (Apêndice D, grade oficial).** `Geometria › Percepção Espacial no Plano › Recorte, Ladrilhamento e Decomposição de Área` (subnó proposto). No Apêndice D, "Geometria" está marcado `[+]`, sem filhos transcritos, então este subnó fica lado a lado com "Reconhecimento e Composição de Figuras Planas" (Bloco $1$) dentro de "Percepção Espacial no Plano".

**Descritores secundários.**
- Multiplicação como contagem de grupos iguais em malha: contar quadradinhos de uma folha recortada é o mesmo raciocínio de multiplicar linhas por colunas já usado em Aritmética, aqui aplicado a uma malha geométrica.
- Divisão como repartição de um total em partes iguais: achar quantas peças cabem numa malha (área total dividida pela área de uma peça) é parente do descritor "processo iterativo / repartição igual" já registrado noutros blocos da trilha.
- Restrição de posição e orientação (sem girar, sem virar): a variação da peça em L exige comparar não só o formato de duas figuras, mas também a direção exata em que cada uma está virada, o mesmo cuidado do Bloco $1$ aplicado agora a peças de ladrilhamento.
- Subtração como resto de uma soma conhecida: tanto a contagem de peças de uma cor quanto a área de uma região que falta se resolvem subtraindo uma soma parcial de um total já determinado.
...
**Campo de trilha.** Trilha Mirim 2 › **Caminho 4** › **Bloco 2 · Quebrar e remontar figuras** › capítulo de teoria "Quebrar e remontar figuras" (`Cap_Figuras_RecortarLadrilhar.md`).
...
Os $10$ exercícios de `Lista_Exercicios_Figuras_RecortarLadrilhar.md` seguem a classificação acima, variando o motor específico dentro do bloco:
- Exercícios $1$, $5$, $9$: variação (a), recorte e contagem de quadradinhos numa folha.
- Exercícios $2$, $6$, $10$: variação (b), ladrilhamento e contagem de peças por cor.
- Exercícios $3$, $7$: variação (b), caso do encaixe por forma e posição fixas, sem girar nem virar.
- Exercícios $4$, $8$: variação (c), área de uma região por decomposição do total.
